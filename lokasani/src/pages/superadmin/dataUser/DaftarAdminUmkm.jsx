            ...
            <tbody>
              {userData.map((item) => (
                <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className={`px-1 py-4 font-semibold ${item.status === 'Aktif' ? 'text-green-500' : 'text-red-500'}`}>
                    {item.status}
                  </td>
                  <td className="px-10 py-4 relative">
                    ...
                    {activeDropdown === item.id && (
                      <div
                        className="absolute left-16 top-[20px] z-20 w-32 py-4 rounded-md shadow-md shadow-gray-400 bg-white"
                      >
